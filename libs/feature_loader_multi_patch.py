    ...
    def setup(self):
        ...
        for idx, video in enumerate(f_lines):
            ...
            with h5py.File(feature_hdf5_path, 'r') as f:
                video_dict[video]['num_frames'] = f['rgb'].shape[-1]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Number of videos
        number_videos = 16
        # The file records all the name videos
        name_videos = ''
        phrase = 'train'

        for i in range(number_videos):
            hdf5_name = (dataset + '_' + phrase + '_%02d.h5') %(i + 1)
            name_videos += hdf5_name + '\n'

        with open('../txt/' + dataset + '_feature_' + phrase + '.txt', 'w') as f:
            f.write(name_videos)

    # test demo
    else:
        feature_path = '/home/luowx/datasets/anomaly_detection/ped1/training/224/features/twostream_res50_7x7'
        video_txt = '../txt/ped1_feature_train.txt'
