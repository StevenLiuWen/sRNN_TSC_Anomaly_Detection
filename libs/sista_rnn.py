...
class sista_rnn(object):
    ...
    def forward_relax1(self):
        ...
        # define variables and constants in the sista_rnn model
        with tf.variable_scope('sista_rnn_trainable_variables'):
            V = self.__get_variable('V', [self.n_hidden, self.n_input])
            U = self.__get_variable('U', [self.n_hidden, self.n_hidden])
            gama = self.__get_variable('gama', None, tf.constant(self.gama, tf.float32))
            lambda1 = self.__get_variable('lambda1', None, tf.constant(self.lambda1, tf.float32))
        parameters = OrderedDict([('V', V),
                                  ('U', U),
                                  ('gama', gama),
                                  ('lambda1', lambda1)])

        with arg_scope([matmul], transpose_b=True):
            h = self.__soft(matmul(self.now_input[0], V), lambda1 / gama)

            for k in range(1, self.K):
                h = self.__soft(matmul(self.now_input[0], V) + matmul(h, U), lambda1 / gama)

            h = tf.expand_dims(h, 0)

        return h, parameters

    def forward_relax2(self):
        '''
        :return: the hidden state with shape: [time_step, batch_size, K * n_hidden],
                 parameters: A dict with A, gama, lambda1, lambda2, h_0
        '''
        ...
        with tf.variable_scope('sista_rnn_trainable_variables'):
            ...
            for i in range(self.K - 1):
                Us.append(self.__get_variable('U_{}'.format(i + 1), [self.n_hidden, self.n_hidden]))
            gama = self.__get_variable('gama', None, tf.constant(self.gama, tf.float32))
            lambda1 = self.__get_variable('lambda1', None, tf.constant(self.lambda1, tf.float32))
        parameters = OrderedDict([('U_{}'.format(i + 1), Us[i]) for i in range(self.K - 1)] +
                                 [('V', V),
                                  ('gama', gama),
                                  ('lambda1', lambda1)])
        ...
        with arg_scope([matmul], transpose_b=True):
            h = self.__soft(matmul(self.now_input[0], V), lambda1 / gama)
            #h = tf.nn.sigmoid(matmul(self.now_input[0], V))

            for k in range(self.K - 1):
                h = self.__soft(matmul(self.now_input[0], V) + matmul(h, Us[k]), lambda1 / gama)
                #h = tf.nn.sigmoid(matmul(self.now_input[0], V) + matmul(h, Us[k]))
    ...
    def forward_relax3(self):
        ...
        with tf.variable_scope('sista_rnn_trainable_variables'):
            for i in range(self.K):
                Vs.append(self.__get_variable('V_{}'.format(i + 1), [self.n_hidden, self.n_input]))
            for i in range(self.K - 1):
                Us.append(self.__get_variable('U_{}'.format(i + 1), [self.n_hidden, self.n_hidden]))
            gama = self.__get_variable('gama', None, tf.constant(self.gama, tf.float32))
            lambda1 = self.__get_variable('lambda1', None, tf.constant(self.lambda1, tf.float32))
        parameters = OrderedDict([('U_{}'.format(i + 1), Us[i]) for i in range(self.K - 1)] +
                                 [('V_{}'.format(i + 1), Vs[i]) for i in range(self.K)] +
                                 [('gama', gama),
                                  ('lambda1', lambda1)])
        ...
        with arg_scope([matmul], transpose_b=True):
            ...
            for k in range(self.K - 1):
                h = self.__soft(matmul(self.now_input[0], Vs[k + 1]) + matmul(h, Us[k]), lambda1 / gama)
